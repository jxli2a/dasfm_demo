"""
Multi-channel cross-correlation for picking polarity, delta_t, and aligning waveforms
Author: Jiaxuan Li @ Caltech. jxli@caltech.edu
"""
import torch
...
class MCCCPicker:
    ...
    def __init__(
        ...
        Args:
            ...
            mccc_damp: weight factor for mccc
        ...
        #self.data = data
        #self.data_freq = self.fft_real_xcor(normalize(torch.clone(data)))
    ...
    def spectral_whitening(self, data_freq):
        ...
        """
        data_freq_energy_1 = torch.mean(
            torch.abs(data_freq) ** 2, dim=-1, keepdim=True
        ).sqrt()
    ...
    def pick_data_shift_win_maxabs(
        self, data_to_pick, dt, maxlag, shift_index, ma=None, update_vmin=False
    ):
        """"""
        if ma is None:
            return self.pick_data_win_maxabs(
                gather_roll(data_to_pick, shift_index),
                dt,
                maxlag,
                update_vmin=update_vmin,
            )
        else:
            return self.pick_data_win_maxabs(
                moving_average(gather_roll(data_to_pick, shift_index), 40),
                dt,
                maxlag,
                update_vmin=update_vmin,
            )
    ...
    def resample_data_in_freq_domain(self, data):
        """
        upsample the data by padding zeros in frequency domain
        ...
        data_freq_pad = F.pad(
            data_freq, (0, ntime_q // 2 - ntime_b // 2), "constant", self.zero_complex
        )
        data_resample = (
            torch.fft.irfft(data_freq_pad, dim=-1, n=ntime_b * self.scale_factor)
            * self.scale_factor
        )
    ...
    def form_A_b_scipy(self, index_i, index_j, mccc_cc, mccc_dt):
        ...
        weight = np.ones(len(mccc_cc)) * self.mccc_damp
        ...
        # difference operator
        D = sparse.coo_matrix(
            (value_ij, (index_ii, index_jj)), shape=(ngood, self.nchan)
        )
        d = np.concatenate([mccc_dt.numpy() * weight, []])
        # smooth operator:  t[i+1] - t[i] = 0
        S = (np.diag(np.ones(self.nchan)) - np.diag(np.ones(self.nchan - 1), k=-1))[
            1:, :
        ]
    ...
    def concat_A_b_with_picking(self, A, b, pick_dt, pick_cc, w):
        ...
        isel = torch.where(
            torch.abs(pick_cc) > torch.quantile(torch.abs(pick_cc), 0.15)
        )[0]
        nsel = len(isel)
        P = sparse.coo_matrix(
    ...
    def pick(self, pick_dt, win=None):
        """
        Pick directly in a narrow window [-win, +win] with known pick_dt
        """
        ...
        # pick main lobe
        pick_cc, _, pick_dt_refine = self.pick_data_shift_win_maxabs(
            data_resample, dt_resample, win, shift_index, ma=self.refine_ma
        )
        # pick side lobe
        pick_side = self.pick_data_win_maxabs_side_lobe(
                data_resample, dt_resample, self.win_side
        )
        sol = {
        ...
        if self.return_data_align:
            sol["data_align"] = moving_average(
                gather_roll(data_resample, shift_index), 40
            )
    ...
    def solve(self):
        ...
        while niter < self.max_niter:
            ...
            if niter == 0:
                ...
                pick_cc, _, pick_dt_refine = self.pick_data_win_maxabs(
                    moving_average(
                        gather_roll(data_resample, shift_index), self.refine_ma
                    ),
                    dt_resample,
                    win_main,
                    update_vmin=False,
            ...
            if self.mode == "pick":
                A, b = self.concat_A_b_with_picking(
